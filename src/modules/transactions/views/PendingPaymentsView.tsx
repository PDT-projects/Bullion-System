...
// UI exactly same as src/features/finance/PendingPayments.tsx

import { useState, useMemo } from 'react';
import { Transaction, PartialPayment } from '../models/types';
import { Eye, Trash2, X, Check, AlertCircle, Clock, DollarSign, Plus, Filter, Printer } from 'lucide-react';
...
export function PendingPaymentsView({ transactions, setTransactions, banks }: PendingPaymentsViewProps) {
  ...
  // Filter to show only pending transactions
  ...
  const addPartialPayment = (transactionId: string) => {
    ...
    setPaymentData({ amount: 0, bankId: '', method: 'Cash', chequeNumber: '' });
  };    
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredTransactions.map((transaction) => (
                  <tr key={transaction.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 text-sm font-medium text-[#4f46e5]">{transaction.transactionId || transaction.id}</td>
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{formatDateTime(viewTransaction.date, viewTransaction.time)}</p>
                </div>
                <div>
              ...
              <div className="border-t pt-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-3 bg-blue-50 rounded">
                    ...
                    <p className="text-lg font-bold text-blue-600">{formatCurrency(viewTransaction.amount)}</p>
                  </div>
                  <div className="p-3 bg-green-50 rounded">
                    <p className="text-xs text-gray-600">Amount Paid</p>
                    <p className="text-lg font-bold text-green-600">{formatCurrency(getTransactionTotals(viewTransaction).totalPaid)}</p>
                  </div>
                  <div className="p-3 bg-red-50 rounded col-span-2">
                    <p className="text-xs text-gray-600">Remaining Amount</p>
                    <p className="text-lg font-bold text-red-600">{formatCurrency(getTransactionTotals(viewTransaction).remainingAmount)}</p>
                  </div>
                </div>
              </div>
                      ...
                      <div key={payment.id} className="p-3 bg-gray-50 rounded border border-gray-200">
                        <div className="flex justify-between items-start">
                          ...
                          <span className={`px-2 py-1 text-xs rounded font-medium ${
                            payment.isCleared ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                          }`}>
                ...
                <>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setPaymentData({ ...paymentData, method: e.target.value as 'Cash' | 'Cheque' | 'Bank' })}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#4f46e5]"
                    >
