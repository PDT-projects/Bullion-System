...
const inp = 'w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-400 focus:border-transparent outline-none bg-white transition-all placeholder:text-gray-400';
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div style={{ width: 40, height: 40, borderRadius: 12, backgroundColor: '#4f46e5', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-5">
              ...
              <div>
                <label className={lbl}>
                  <span className="flex items-center gap-1"><DollarSign size={10} /> Price (PKR)</span>
                </label>
            ...
            <div className="flex justify-end pt-2 border-t border-gray-100">
              <button
                ...
                style={{
                  ...
                  padding: '10px 24px', borderRadius: 8, border: 'none', cursor: isSubmitting ? 'not-allowed' : 'pointer',
                  backgroundColor: isSubmitting ? '#a5b4fc' : '#4f46e5', color: '#ffffff',
                  fontSize: 14, fontWeight: 700, boxShadow: '0 2px 6px rgba(79,70,229,0.35)',
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-sm font-bold text-gray-900">
                  All Assets
                  <span className="ml-2 px-2 py-0.5 bg-indigo-50 text-indigo-600 text-xs font-bold rounded-full">{assets.length}</span>
                </h2>
            ...
            {assets.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {assets.map((asset) => (
                  <div
                    key={asset.id}
                    className="group relative border border-gray-200 rounded-xl p-4 hover:border-indigo-200 hover:shadow-md transition-all bg-white"
                  >
                    {/* Card top */}
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <span className="text-xs font-semibold text-indigo-600">{AssetsService.formatPrice(asset.price)}</span>
                        </div>
                      </div>
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0 ml-2">
                        <button onClick={() => handleEdit(asset)}
                          className="p-1.5 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-600 transition-colors" title="Edit">
                          <Edit2 size={12} />
                        </button>
                    ...
                    {expandedId === asset.id && editingAsset?.id === asset.id && (
                      <div className="mt-3 pt-3 border-t border-gray-100 space-y-2">
                        {[
                          ...
                          { value: String(editingAsset.price), placeholder: 'Price', type: 'number',
                          ...
                          <input key={i} type={type}
                            ...
                            className="w-full px-3 py-1.5 text-xs border border-gray-200 rounded-lg focus:ring-1 focus:ring-indigo-400 outline-none" />
                        ))}
                        <div className="flex gap-2 pt-1">
                          <button onClick={handleSaveEdit}
                            className="flex-1 flex items-center justify-center gap-1 bg-indigo-600 hover:bg-indigo-700 text-white py-1.5 rounded-lg text-xs font-semibold transition-colors">
                            <Check size={12} /> Save
                          </button>
          ...
          {assets.length > 0 && (
            <div className="px-6 py-3 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
              ...
              <span className="text-xs text-gray-400">
                Total value: <span className="font-semibold text-indigo-600">
                  {AssetsService.formatPrice(assets.reduce((s, a) => s + a.price, 0))}
                </span>
              </span>
            </div>
          )}
