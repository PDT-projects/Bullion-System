// Payroll Module - Salary List ViewModel
...
import { SalaryService } from '../models/payrollService';
...
export function useSalaryListViewModel(): UseSalaryListViewModelReturn {
  ...
  const [filters, setFilters]         = useState<SalaryFilters>({
    searchTerm: '', typeFilter: 'all', dateFrom: null, dateTo: null,
    employeeFilter: '', monthFilter: '', paymentMethodFilter: '',
  });
  ...
  const salaries            = useMemo(() => SalaryService.filterSalaries(allSalaries, filters), [allSalaries, filters]);
  const stats               = useMemo(() => SalaryService.calculateStats(salaries), [salaries]);
  const activeFilterCount   = useMemo(() => SalaryService.countActiveFilters(filters), [filters]);
  const uniqueEmployees     = useMemo(() => SalaryService.getUniqueEmployees(allSalaries), [allSalaries]);
  const uniqueMonths        = useMemo(() => SalaryService.getUniqueMonths(allSalaries), [allSalaries]);

  const setFilter     = useCallback((key: keyof SalaryFilters, value: any) => setFilters(prev => ({ ...prev, [key]: value })), []);
  const clearFilters  = useCallback(() => setFilters({ searchTerm: '', typeFilter: 'all', dateFrom: null, dateTo: null, employeeFilter: '', monthFilter: '', paymentMethodFilter: '' }), []);
  ...
  const handleAdd  = useCallback((type: 'regular' | 'advance') =>
    navigate(type === 'regular' ? '/payroll/salary/create-regular' : '/payroll/salary/create-advance'), [navigate]);

  const handleEdit = useCallback((id: string) => navigate(`/payroll/salary/${id}/edit`), [navigate]);
  const handlePrint = useCallback(() => window.print(), []);

  const getSalaryTypeColor  = useCallback((subCategory: string) => SalaryService.getSalaryTypeColor(subCategory), []);
  const getEmployeeTotalPaid = useCallback((employeeId: string, month: string) =>
    SalaryService.getEmployeeTotalPaid(allSalaries, employeeId, month), [allSalaries]);
  const isEmployeeFullyPaid  = useCallback((employeeId: string, month: string, fullSalary: number) =>
    SalaryService.isEmployeeFullyPaid(allSalaries, employeeId, month, fullSalary), [allSalaries]);

  return {
    salaries, allSalaries, filters, showFilters, activeFilterCount,
    viewingSalary, isLoading,
    stats: {
