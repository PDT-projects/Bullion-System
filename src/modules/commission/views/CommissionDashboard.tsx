...
import { Check, AlertTriangle, Calendar, DollarSign, User, TrendingUp, Award, FileText, Loader2 } from 'lucide-react';
...
export function CommissionDashboard({ invoices, employees, setActiveTab }: DashboardProps) {
  ...
  useEffect(() => {
    ...
    setTotalPendingCommission(pending.reduce((sum, c) => sum + (c.calculatedCommissionAmount || 0), 0));

    // Recent 5 commissions (all statuses)
    setRecentCommissions(liveCommissions.slice(0, 5));
  }, [liveCommissions]);

  const handleQuickConfirmCurrentMonth = async () => {
    if (pendingCount === 0) {
      toast.info('No pending commissions for current month');
      return;
    }

    try {
      const currentMonth = new Date().toISOString().slice(0, 7);
      const currentCity = 'Karachi'; // Default or from user pref
      const success = await calculateCommission(invoices, employees);
      if (success) {
        confirmAllCommissions();
        toast.success(`Confirmed ${pendingCount} pending commissions for ${currentCity} (${formatMonth(currentMonth)})`);
        refreshLiveCommissions();
      }
    } catch (error) {
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg shadow-sm border border-blue-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <button
            ...
            className="group flex flex-col items-center p-6 border-2 border-dashed border-orange-300 rounded-lg hover:border-orange-400 hover:bg-orange-50 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-transparent"
          >
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-max">
              <thead className="bg-orange-50 border-b border-orange-200">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-orange-800 uppercase tracking-wider min-w-[150px]">
                    Salesperson
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-orange-800 uppercase tracking-wider min-w-[100px]">
                    City
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-orange-800 uppercase tracking-wider min-w-[100px]">
                    Month
                  </th>
                  <th className="px-6 py-3 text-right text-xs font-semibold text-orange-800 uppercase tracking-wider min-w-[120px]">
                    Total Sales
                  </th>
                  <th className="px-6 py-3 text-right text-xs font-semibold text-orange-800 uppercase tracking-wider min-w-[120px]">
                    Commission
                  </th>
                  <th className="px-6 py-3 text-right text-xs font-semibold text-orange-800 uppercase tracking-wider min-w-[100px]">
                    Invoices
                  </th>
                  <th className="px-6 py-3 text-center text-xs font-semibold text-orange-800 uppercase tracking-wider min-w-[140px]">
                    Calculated
                  </th>
                </tr>
              </thead>
            ...
            <div key={commission.id} className="px-6 py-4 hover:bg-gray-50 transition-colors">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-3 h-3 rounded-full ${commission.status === 'Confirmed' ? 'bg-green-500' : commission.status === 'Adjusted' ? 'bg-yellow-500' : 'bg-blue-500'}`} />
                  <div>
