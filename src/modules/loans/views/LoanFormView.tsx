...
import { ArrowLeft, Save, X, AlertCircle, Building2, User, Phone, Calendar, Wallet, Landmark } from 'lucide-react';
...
import { sanitizeNameInput, sanitizePhoneInput } from '../../../utils/validators';
...
/** Inline AED prefix icon — replaces the generic DollarSign for all amount fields */
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="space-y-6">
            ...
            {(['Payable', 'Receivable'] as LoanType[]).map(t => (
              <label
                ...
                className={`flex items-center p-4 border-2 rounded-lg cursor-pointer transition-all ${
                  formData.type === t
                    ? (t === 'Payable' ? 'border-red-500 bg-red-50' : 'border-green-500 bg-green-50')
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="text-sm text-gray-500">
                    {t === 'Payable' ? 'Money we need to pay to others' : 'Money others owe to us'}
                  </div>
                </div>
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                  formData.type === t
                    ? (t === 'Payable' ? 'border-red-500' : 'border-green-500')
                    : 'border-gray-300'
                }`}>
                  {formData.type === t && (
                    <div className={`w-3 h-3 rounded-full ${t === 'Payable' ? 'bg-red-500' : 'bg-green-500'}`} />
                  )}
                </div>
              </label>
            ))}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={e => setEntityName(sanitizeNameInput(e.target.value))}
                  placeholder="Enter entity name (letters only)"
                  className={`w-full pl-10 pr-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#4f46e5] ${
        ...
        {/* Amounts */}
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-medium text-gray-900 mb-4">Amount Details (AED)</h3>
                ...
                {(['Person', 'Employee'] as ReceiverType[]).map(rt => (
                  <label key={rt} className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="radio"
                      value={rt}
                      checked={formData.receiverType === rt}
                      onChange={() => setReceiverType(rt)}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={e => setReceiverName(sanitizeNameInput(e.target.value))}
                      placeholder="Enter receiver name (letters only)"
                      className={`w-full pl-10 pr-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#4f46e5] ${
