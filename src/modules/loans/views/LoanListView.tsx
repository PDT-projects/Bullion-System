...
import type { Loan, LoanFilters, LoanSortField, SortOrder, LoanType } from '../models/types';
...
interface LoanListViewProps {
  ...
  setStatusFilter: (status: 'Full' | 'Partial' | 'all') => void;
  setCategoryFilter: (category: 'Official' | 'Personal' | 'Other' | 'all') => void;
  clearFilters: () => void;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            {totalCount} loans • Total: {formatCurrency(totalAmount)}
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setStatusFilter(e.target.value as 'Full' | 'Partial' | 'all')}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#4f46e5] focus:border-transparent"
              >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setCategoryFilter(e.target.value as 'Official' | 'Personal' | 'Other' | 'all')}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#4f46e5] focus:border-transparent"
              >
              ...
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th 
                    ...
                    onClick={() => setSortField('entityName')}
                  >
                    Entity {sortField === 'entityName' && (sortOrder === 'asc' ? '↑' : '↓')}
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Type</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Category</th>
                  <th 
                    className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                    onClick={() => setSortField('loanAmount')}
                  >
                    Amount {sortField === 'loanAmount' && (sortOrder === 'asc' ? '↑' : '↓')}
                  </th>
                  <th 
                    className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                    onClick={() => setSortField('paid')}
                  >
                    Paid {sortField === 'paid' && (sortOrder === 'asc' ? '↑' : '↓')}
                  </th>
                  <th 
                    className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                    onClick={() => setSortField('remaining')}
                  >
                    Remaining {sortField === 'remaining' && (sortOrder === 'asc' ? '↑' : '↓')}
                  </th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loans.map((loan) => (
                  <tr key={loan.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${categoryColors[loan.loanType]}`}>
                        {loan.loanType}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          ...
                          className="p-1 hover:bg-gray-100 rounded transition-colors text-green-600"
                          title="Edit"
                        >
                        ...
                        {loan.status !== 'Full' && (
                          <button
                            ...
                            className="p-1 hover:bg-gray-100 rounded transition-colors text-purple-600"
                            title="Make Payment"
                          >
