...
export const validateLoan = (dto: CreateLoanDTO): LoanValidationErrors => {
  ...
  const entityVal = validateName(dto.entityName, 'Entity name', true);
  if (!entityVal.isValid) errors.entityName = entityVal.error!;
  ...
  if (dto.receiverType === 'Person') {
    const rxVal = validateName(dto.receiverName, 'Receiver name', true);
    if (!rxVal.isValid) errors.receiverName = rxVal.error!;
  } else if (!dto.receiverName || dto.receiverName.trim().length === 0) {
    errors.receiverName = 'Receiver is required';
  }

  if (dto.receiverPhone?.trim()) {
    const phoneVal = validatePhone(dto.receiverPhone, 'Receiver phone', false);
    if (!phoneVal.isValid) errors.receiverPhone = phoneVal.error!;
  }
