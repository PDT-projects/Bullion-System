...
export function PayableLoansPage() {
  ...
  const payableLoans = loans.filter(loan => loan.type === 'Payable');

  // Apply additional filters
  const filteredLoans = payableLoans.filter(loan => {
    const matchesSearch = 
  ...
  const stats = {
    totalLoans: payableLoans.length,
    totalAmount: payableLoans.reduce((sum, loan) => sum + loan.loanAmount, 0),
    totalPaid: payableLoans.reduce((sum, loan) => sum + loan.paid, 0),
    totalRemaining: payableLoans.reduce((sum, loan) => sum + loan.remaining, 0),
    fullCount: payableLoans.filter(l => l.status === 'Full').length,
    partialCount: payableLoans.filter(l => l.status === 'Partial').length
  };
      ...
      if (loanToDelete.mode === 'Bank' && loanToDelete.bankId && setBanks) {
        const updatedBanks = banks.map(bank => {
          if (bank.id === loanToDelete.bankId) {
            const adjustment = -loanToDelete.loanAmount; // Reverse the money that came in
    ...
    if (paymentMode === 'Bank' && bank && setBanks) {
      const updatedBanks = banks.map(b => {
        if (b.id === paymentBankId) {
          return { ...b, balance: b.balance - paymentAmount }; // Money goes OUT
        }
        return b;
      });
      setBanks(updatedBanks);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600">Loans taken from companies or individuals that need to be repaid</p>
          </div>
        </div>
        <button
      ...
      <div className="flex flex-wrap items-center gap-4 bg-white p-4 rounded-lg border border-gray-200">
        <div className="flex-1 min-w-[200px] relative">
          ...
          <input
            type="text"
            placeholder="Search by entity name, bank..."
            value={searchTerm}
            ...
            {filteredLoans.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-4 py-12 text-center text-gray-500">
                  ...
                  <p className="text-lg font-medium">No payable loans found</p>
                  <p className="text-sm mt-1">Create a new payable loan to get started</p>
                </td>
              </tr>
            ) : (
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{loan.loanType} • {loan.mode === 'Bank' ? loan.bankName : 'Cash'}</p>
                      </div>
                    </div>
                  </td>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium text-lg">{viewingLoan.entityName}</p>
                </div>
                <div>
